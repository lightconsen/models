...
| [ ] | `volcesark` | official | 0/11 | <https://www.volcengine.com/> | the plan's own model list — Auto plus ten named models — and the two doc pages agree on the base URLs the entry already carried. `doubao-seed-2-1-pro` and its dated snapshot were carried here and the vendor says other models cannot be used in the Coding Plan at all; its rate was a metered one, which is why nothing is priced here now. No model is priced only here, so the price table moved only by that row |
| [ ] | `volcesark-agent-plan` | official | 0/11 | <https://www.volcengine.com/> | eleven text models from the plan's own table, all unpriced — the plan deducts quota rather than charging per token. The non-text rows (embedding, image, video, audio) and the platform features are excluded on the same rule as everywhere else. **The table also marks which tiers get which model** — `kimi-k3` is absent from Small — and no field holds that |
| [ ] | `volcesark-payg` | official | 11/11 | <https://www.volcengine.com/> | the metered rates from the vendor's own page, in yuan — the same page confirms the `doubao-seed-2.1-pro` rate that was removed from the Coding Plan entry. Current generation only: the 1.5/1.6/1.8 rows are previous generations and the 调整前价格 rows are the same models at old prices. **`long_context` cannot express what four of these models do** — doubao-seed-2.0-pro, -lite, -mini and -code each have three bands by input length (0-32k, 32-128k, 128-256k) and the field holds one threshold; the first band is recorded and the other two are not. Also published here with no field to hold them: cache storage at 0.017 元/百万token/小时, the Coding Plan at 40/200 元/月 and the Agent Plan at 40/200/500/1000 元/月 — the third-party figures quoted earlier for the Coding Plan (68/38) were wrong |
