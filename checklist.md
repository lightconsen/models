...
Fifteen of the twenty-two entries can be read by script rather than by eye. **Run `node scripts/fetch-all.mjs` before reading anything** — it reads every source, prints what each would change, and decides whether a person is needed. `--write` applies, `--write --commit` applies and commits one entry per commit. The per-vendor `fetch-*.mjs` scripts still work and are the better place to read about any single vendor; `fetch-openrouter-rankings.mjs` is still the only one that decides *which* models OpenRouter carries, and with `fetch-aliyun-models.mjs` for the Alibaba model lists it is the only one needing a key.

Seven entries are read by eye rather than by the runner, for two different reasons. Four publish no per-token rate at all — `kimi-for-coding` (a membership) and the `qianwenai-token-plan`, `tencent-token-plan` and `baidu-qianfan-token-plan` plans. Three more have a rate but no page a script can reach: `openai`, geographically blocked from here; `google-gemini`, whose pricing page answers a request with an OAuth consent screen; and `mistral`, whose API rates come off a page this run has not found a stable machine-readable form of. The runner names all seven every run rather than letting their absence read as coverage.
...
| [x] | `openai` | official | 5/5 | <https://openai.com/> | checked 2026-09-15 — **the one entry whose rates are not read from the vendor's own page**, and the exception is deliberate. `platform.openai.com/docs/pricing` and `openai.com/api/pricing` both answer 403 from here, and `api.openai.com` says why: `unsupported_country_region_territory`. That is a geographical block rather than a scraping defence, and no header or path reaches past it. The five current text models — GPT-6 Astra, GPT-5.6, -Sol, -Terra, -Luna — are taken from `models.dev/labs/openai` instead. The conversion risk that made models.dev unusable for the Alibaba entries does not arise here, because OpenAI bills in USD and a USD catalogue has nothing to convert; the rates are also internally consistent with this repo's Anthropic entry, where a cache write is likewise 1.25× input and a read 10%. What remains is that they are a third party's transcription, which is a weaker claim than every other row here makes — so if anyone can reach the vendor's page, replacing these five is the way to close it. `billing` is `payg`: the API bills per token against prepaid credit, and the consumer subscriptions are a different product at a different address that grants no API quota |
