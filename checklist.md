...
| [x] | `google-ai-studio` | official | 3/4 | <https://aistudio.google.com/> | checked 2026-09-14 — the three Flash rates and the model list from the vendor's own pricing and models pages; `desc` was the placeholder. The endpoint answers: 403 without a key and 400 "API key not valid" with a bogus one, which is the route reading the header — and both `/v1beta/interactions` and the classic `:generateContent` path are live |
