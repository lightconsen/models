...
| [ ] | `minimax` | official | 2/2 | <https://minimaxi.com/> | the vendor prices three language models — M2.7, M2.7-highspeed and M3 — and lists eight; M3 is not carried because it is priced in two bands by input length, which no field holds. `billing` is `both`: one address takes a pay-as-you-go key and a Token Plan key |
